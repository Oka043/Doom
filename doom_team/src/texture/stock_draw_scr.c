...
void DrawScreen(t_player *player, unsigned *NumSectors, SDL_Surface *surface, t_sector *sectors, t_system *system)
{
	...
	*head = (struct item) { player->num_sect, 0, W-1 };
	...
	while(head != tail)
	{
		...
		t_sector* const sect = &sectors[now.sectorno];
...
//#ifdef TextureMapping
				t_xy org1 = {tx1,tz1}, org2 = {tx2,tz2};
//#endif
...
#define RelativeMapCoordinatesToAbsoluteOnes(X,Z) \
				...
				vline(x, ytop[x], cya-1, 0x111111 ,0x222222,0x111111, system);
				/* Render floor: everything below this sector's floor height. */
				vline(x, cyb+1, ybottom[x], 0x0000FF,0x0000AA,0x0000FF, system);
#endif
				...
				if(neighbour >= 0)
				{
					...
					vline(x, cya, cnya-1, 0, x==x1||x==x2 ? 0 : r1, 0, system);
#endif
...
#else
					vline(x, cnyb+1, cyb, 0, x==x1||x==x2 ? 0 : r2, 0, system);
#endif
...
#else
					unsigned r = 0xAAAAAA;
					vline(x, cya, cyb, 0, x==x1||x==x2 ? 0 : r, 0, system);
#endif
				}
