...
/* Sectors: Floor and ceiling height; list of edge vertices and neighbours */
static struct sector
{
	float		floor;
	float		ceil;
	t_vector	*vertex; 	 // Each vertex has an x and y coordinate
	signed char	*neighbours;  // Each edge may have a corresponding neighbouring sector
...
t_player player;
t_camera camera = {1e-4f, 5.f, 1e-5f, 20.f, HFOV, VFOV};
t_window main_win;
...
static void UnloadData()
{
	...
	for(unsigned a=0; a<NumSectors; ++a) free(sectors[a].neighbours);
	free(sectors);
	sectors    = NULL;
	NumSectors = 0;
}
...
static void MovePlayer(float dx, float dy)
{
	...
	const t_vector* const vert = sect->vertex;
	...
	*head = (struct item) { player.sector, 0, W-1 };
	...
	do {
		...
		for(unsigned s = 0; s < sect->npoints; ++s)
		{
			...
			float tx1 = vx1 * pcos - vy1 * psin,  tz1 = vx1 * psin + vy1 * pcos;
			float tx2 = vx2 * pcos - vy2 * psin,  tz2 = vx2 * psin + vy2 * pcos;
			...
			if(tz1 <= 0 || tz2 <= 0)
			{
				...
				t_vector i1 = Intersect(tx1,tz1,tx2,tz2, -nearside,nearz, -farside,farz);
				t_vector i2 = Intersect(tx1,tz1,tx2,tz2,  nearside,nearz,  farside,farz);
			...
			/* Check the edge type. neighbour=-1 means wall, other=boundary between two sectors. */
			int neighbour = sect->neighbours[s];
			float nyceil=0, nyfloor=0;
			if(neighbour >= 0) // Is another sector showing through this portal?
			{
				nyceil  = sectors[neighbour].ceil  - player.where.z;
				nyfloor = sectors[neighbour].floor - player.where.z;
			}
...
#define Yaw(y,z) (y + z*player.yaw)
			...
			/* The same for the neighbouring sector */
			...
			for(int x = beginx; x <= endx; ++x)
			{
				...
				{
					/* There's no neighbour. Render wall from top (cya = ceiling level) to bottom (cyb = floor level). */
					unsigned r = 0x010101 * (255-z);
					vline(x, cya, cyb, 0, x==x1||x==x2 ? 0 : r, 0);
				}
				SDL_UpdateWindowSurface(window);
			}
			/* Schedule the neighbouring sector for rendering within the window formed by this wall. */
...
static void DrawScreen()
{
	...
	struct sector *const sect = &sectors[now.sectorno];
	for(unsigned s = 0; s < sect->npoints; ++s)
	{
		/* Acquire the x,y coordinates of the two endpoints (vertices) of this edge of the sector */
		t_vector vx1 = V_SUB(sect->vertex[s + 0], player.where);
		t_vector vx2 = V_SUB(sect->vertex[s + 1], player.where);
		/* Rotate them around the player's view */
		t_vector tx1 = v2_rotate(vx1, player.anglecos, player.anglesin);
		t_vector tx2 = v2_rotate(vx2, player.anglecos, player.anglesin);
		/* Is the wall at least partially in front of the player? */
		if(tx1.y <= 0 && tx2.y <= 0)
			continue;
		/* Do perspective transformation */
		t_vector line = line_to_screen(&camera, &tx1, &tx2);
		float yscale1 = camera.vfov / tx1.y;
		float yscale2 = camera.vfov / tx2.y;

		if(line.x >= line.y || line.y < now.sx1 || line.x > now.sx2)
		...
		int neighbour = sect->neighbours[s];
		/* Project our ceiling & floor heights into screen coordinates (Y coordinate) */
		int y1a  = H/2 - (int)(Yaw(yceil, tx1.y) * yscale1),  y1b = H/2 - (int)(Yaw(yfloor, tx1.y) * yscale1);
		int y2a  = H/2 - (int)(Yaw(yceil, tx2.y) * yscale2),  y2b = H/2 - (int)(Yaw(yfloor, tx2.y) * yscale2);
		/* Render the wall. */
			int beginx = MAX(line.x, now.sx1), endx = MIN(line.y, now.sx2);
			for(int x = beginx; x <= endx; ++x)
			{
				/* Acquire the Y coordinates for our ceiling & floor for this X coordinate. Clamp them. */
				int ya = (x - line.x) * (y2a-y1a) / (line.y-line.x) + y1a, cya = CLAMP(ya, ytop[x],ybottom[x]); // top
				int yb = (x - line.x) * (y2b-y1b) / (line.y-line.x) + y1b, cyb = CLAMP(yb, ytop[x],ybottom[x]); // bottom
				...
				if(neighbour >= 0)
				{
					vline(x, cya, cyb, 0x00AA00,0x00AA00,0x00AA00);
				}
				else
				{
					/* There's no neighbour. Render wall from top (cya = ceiling level) to bottom (cyb = floor level). */
					vline(x, cya, cyb, 0, x==line.x||x==line.y ? 0 : 0xAAAAAA, 0);
				}
				//SDL_UpdateWindowSurface(window);
			}
		} // for s in sector's edges
...
//	surface = SDL_SetVideoMode(W, H, 32, 0);
	...
	for(;;)
	{
		...
		if(moving)
		{
			...
			for(unsigned s = 0; s < sect->npoints; ++s)
				   ...
				   && PointSide(px+dx, py+dy, vert[s+0].x, vert[s+0].y, vert[s+1].x, vert[s+1].y) < 0)
				{
					...
					float hole_low  = sect->neighbours[s] < 0 ?  9e9 : MAX(sect->floor, sectors[sect->neighbours[s]].floor);
					float hole_high = sect->neighbours[s] < 0 ? -9e9 : MIN(sect->ceil,  sectors[sect->neighbours[s]].ceil );
		...
		if(wsad[0]) { move_vec[0] += player.anglesin*0.2f; move_vec[1] += player.anglecos*0.2f; }
		if(wsad[1]) { move_vec[0] -= player.anglesin*0.2f; move_vec[1] -= player.anglecos*0.2f; }
		if(wsad[2]) { move_vec[0] += player.anglecos*0.2f; move_vec[1] -= player.anglesin*0.2f; }
		if(wsad[3]) { move_vec[0] -= player.anglecos*0.2f; move_vec[1] += player.anglesin*0.2f; }
...
//				sect->visible = 0;
//#endif
//				for(n=0; n<m; ++n) sect->neighbours[n] = numbers[m + n];
//				for(n=0; n<m; ++n)
//				{
//					int v = numbers[n];
...
//		free(sectors[a].vertex), free(sectors[a].neighbours);
//	free(sectors);
//	sectors    = NULL;
//	NumSectors = 0;
//}
...
//				}
//			}
//			if(sect->neighbours[b] >= 0 && sect->neighbours[b] < (int)NumSectors && found != 1)
...
//				/* Perspective-transform the floor and ceiling coordinates of the neighbouring sector. */
//				nyceil  = sectors[neighbour].ceil  - player.where.z;
//				nyfloor = sectors[neighbour].floor - player.where.z;
//			}
//			int ny1a = H/2 + (int)( -Yaw(nyceil, tz1) * yscale1), ny1b = H/2 + (int)( -Yaw(nyfloor, tz1) * yscale1);
...
//				else
//				{
//					/* There's no neighbour. Render wall. */
//#ifdef TextureMapping
...
//				//*tail = (struct item) { neighbour, beginx, endx };
//			}
//		}
//		++renderedsectors[now.sectorno];
//#ifdef VisibilityTracking
