...
{
	system->dscreen.neighbor = system->dscreen.sect->neighbors[system->dscreen.s];
	system->dscreen.nyceil = 0;
	system->dscreen.nyfloor = 0;
	if (system->dscreen.neighbor >= 0)
	{
		system->dscreen.nyceil = system->sectors[system->dscreen.neighbor].ceil
				- player->where.z;
		system->dscreen.nyfloor = system->sectors[system->dscreen.neighbor].floor
				- player->where.z;
	}
...
void    schedule_sector(t_system *system)
{
	...
	{
		*system->dscreen.head = (t_item) {system->dscreen.neighbor,
