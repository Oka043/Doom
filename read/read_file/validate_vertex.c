...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/12 13:10:30 by olaktion          #+#    #+#             */
/*   Updated: 2019/06/12 13:10:31 by olaktion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool		validate_vertex(char *vertex_data)
{
	...
	if (!ft_strequ(vertex_data, "vertex"))
	{
		...
		if (!validate_dilims(data) || !validate_coords(data)) //вылетает на coords
			print_error("Error, bad vertex validation2");
	}
	else
...
t_bool		validate_coords(char *vertex_data)
{
	...
	while (vertex_data[c])
	{
		...
		{
			if (!ft_isdigit(vertex_data[c]) || !validate_num(&vertex_data[c])) // вылетает val_num
				return (false);
			else
			{
				num++;
				while (vertex_data[c] && (ft_isdigit(vertex_data[c]) || vertex_data[c] == '.'))
					c++;
				continue ;
			}
		}
